...
class _SSMScoreScreenState extends State<SSMScoreScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (scores == null)
      return const Scaffold(body: Center(child: Text('Score not found')));
  ...
  Widget _scoreHeader(Map s) => Container(
    ...
    child: Column(
      children: [
        ...
        Text(
          '${(s['grand_total'] ?? 0).toStringAsFixed(0)} / 500',
          style: const TextStyle(
            color: Colors.white,
            fontSize: 32,
            fontWeight: FontWeight.bold,
        ...
        Row(
          ...
          children: List.generate(
            5,
            (i) => Icon(
              i < (s['star_rating'] ?? 0) ? Icons.star : Icons.star_border,
              color: Colors.white,
            ),
          ),
        ),
  ...
  Widget _cat(String l, dynamic v, Color c) => Card(
    child: ListTile(
      title: Text(l),
      trailing: Text(
        '${(v ?? 0).toInt()} / 100',
